...
import Trap from "./scripts/classes/Trap.mjs";
...
export const params = {
  gameStatus: "menu", // started, menu, paused
  showCollision: false,
  deltaTime: 1,
  time: 0,
...
traps.push(
  new Trap({
    startX: 100, // Початкова позиція попереджувального знака
    startY: 100,
    targetX: 800, // Кінець карти
    targetY: 100,
  })
);
traps.push(
...
const createBurst = ({ subject, type, r, color }) => {
  let burst = [];
  let particleCount =
    params.graphic === "high" ? 450 : params.graphic === "low" && 200;
  for (let i = 0; i < particleCount; i++) {
    burst.push(
      new BurstParticle({
        centerX: subject.position.x + subject.size.width / 2,
        centerY: subject.position.y + subject.size.height / 2,
        type,
        r,
        color,
      })
...
const checkCollision = () => {
  missiles.forEach((missile, index1) => {
    //! missile with Player
    players.forEach((player, index2) => {
      if (
        !missile.ownerId ||
        missile.ownerPhysics ||
        (!missile.ownerPhysics && missile.ownerId !== player.id)
      ) {
        if (
          circleIntersect(
            player.position.x,
    ...
    missiles.forEach((missile, indexM) => {
      if (
        ...
        for (let i = 0; i < particleCount; i++) {
          burst.push(
            new BurstParticle({
              centerX: missile.position.x + missile.size.width / 2,
              centerY: missile.position.y + missile.size.height / 2,
              type: "default",
            })
          );
        }
...
let lastTime = 0;
const targetFps = 60; // Цільова частота кадрів (FPS)
const interval = 1000 / targetFps; // Інтервал між кадрами в мілісекундах
...
const animate = (timestamp) => {
  ...
  if (deltaTime > interval) {
    ...
    planeBursts.forEach((burst) => {
      burst.forEach((part) => {
        planeBurstParticles.push(
          new BurstParticle({
            centerX: part.position.x + part.size.width / 2,
            centerY: part.position.y + part.size.height / 2,
            type: "plane",
            time: part.time,
          })
        );
